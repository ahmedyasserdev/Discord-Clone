...
const ServerHeader = ({server , role}: ServerHeaderProps) => {
  ...
  return (
    ...
    <DropdownMenuContent className = "w-56 p-medium-16 text-black dark:text-neutral-400 space-y-[2px]">
      ...
      {isAdmin && (
          <DropdownMenuItem className="px-3 py-2 p-regural-14 cursor-pointer">
            Manage members
            <Users className="size-4 ml-auto" />
          </DropdownMenuItem>
        )}
              {isModerator && (
          <DropdownMenuItem className="px-3 py-2 p-regular-14 cursor-pointer">
            Create channel
...

        {
          isModerator && (
           ...
           {isAdmin && (
          <DropdownMenuItem className="text-rose-500 px-3 py-2 p-regurla-14 cursor-pointer">
            Delete server
...
{!isAdmin && (
          <DropdownMenuItem className="text-rose-500 px-3 py-2 p-regular-14 cursor-pointer">
            Leave server
